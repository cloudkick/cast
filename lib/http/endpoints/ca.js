...
function getRequest(req, res) {
  ...
  if (!validHostname(hostname)) {
    castHttp.returnError(res, 400, new Error('Invalid hostname'));
    return;
  }
...
function createRequest(req, res) {
  ...
  if (!req.buffer) {
    castHttp.returnError(res, 500, new Error('Missing data'));
    return;
  }

  castHttp.returnReadyJob(res, control.ca.createRequest(hostname, req.buffer));
}
...
function register(app, apiVersion) {
  ...
  app.put('/:hostname/', bodySizeLimiter(MAX_CSR_BYTES), createRequest);
