...
function InvalidSizeError() {
  ...
  this.message = 'Invalid byte length';
  this.responseCode = 400;
}

util.inherits(InvalidSizeError, Error);
...
function tailService(req, res) {
  ...
  if (isNaN(bytes)) {
    castHttp.returnError(res, new InvalidSizeError());
    return;
  }
...
function tailFollowService(req, res) {
  ...
  function onData(err, data, _unsubscribe) {
    if (firstChunk) {
    ...
    if (firstChunk) {
      ...
      req.connection.on('close', unsubscribe);
      firstChunk = false;
    }
