...
function listApplications(req, res) {
  control.bundles.listApplications(function(err, appList) {
    if (err) {
      castHttp.returnError(res, err);
    } else {
      castHttp.returnJson(res, 200, appList);
    }
  });
}
...
function getApplication(req, res) {
  ...
  control.bundles.getApplication(app, function(err, appData) {
    if (err) {
      castHttp.returnError(res, err);
    } else {
      castHttp.returnJson(res, 200, appData);
    }
  });
}
...
function addBundle(req, res) {
  ...
  if (!version) {
    castHttp.returnError(res, 404, new Error('Invalid bundle path'));
    return;
  }
