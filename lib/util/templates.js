 ...
 * @param {Function} callback Callback which is called with a possible error as the first argument and the context object as the second one
 *                            on success
 */
 ...
 * @param {Function} callback Callback which is called with a possible error as a first argument and the rendered template String as the
                              secong argument on success
 */
...
var render_and_save_templates = function(templates_path, template_list, target_path, context, callback) {
  ...
  async.forEach(template_list, function(template_file, callback) {
    ...
    render_template_as_string(template_path, context, function(error, rendered_template) {
      ...
      misc.template_to_tree(target_path, directory_tree_object, true, function(error) {
        return callback(error);
      });
    });
  },
