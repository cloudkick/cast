 ...
 */
exports.get_styles_length = function(string) {
  var i, style_names, styles_count, style;
  var styles_length = 0;


  style_names = Object.keys(styles);
  styles_count = style_names.length;
  for (i = 0; i < styles_count; i++) {
    style = style_names[i];
    if (string.indexOf(sprintf('[%s]', style)) !== -1 &&
        string.indexOf(sprintf('[/%s]', style)) !== -1) {
      styles_length += (style.length + 5);
    }
  }

  return styles_length;
};
...
exports.lpad = function(str, width) {
  ...
  str = String(str);
  styles_length = exports.get_styles_length(str);
  n = (width + styles_length) - str.length;
...
exports.print_table = function(columns, rows, no_data_text) {
  ...
  else
  {
    columns.forEach(function(column) {
      ...
      string = exports.lpad(column_title, padding_left);
      string = exports.rpad(string, column.padding_right);
    ...
    rows.forEach(function(row) {
      columns.forEach(function(column) {
        ...
        string = exports.lpad(value, column.padding_left);
        string = exports.rpad(string, column.padding_right);
...
exports.print_wrapped = function(text, width, indent, output_function) {
  var chunk, styles_length, split_index, remaining, indentstr;
  width = width || 80;
  indent = indent || 2;
  output_function = output_function || sys.puts;
  styles_length = exports.get_styles_length(text);

  indentstr = new Array(indent + 1).join(' ');
  width = width + styles_length;
  remaining = text;
...
/*
 * Replaces style tags with string provided by the replace_function.
 *
 ...
 * @param {Function} replace_function Function which is called with matched style tags and the 
 *                                    text in-between. The function needs to return a string which
 *                                    replaces the provided text.
 *
 * @return {String} String which has been iterated over and passed to the replace_function.
*/
exports.format_tags = function(string, replace_function) {
  var stylized = string;
...
exports.stylize = function(string) {
  ...
  return exports.format_tags(string, replace_function);
};
...
*/
exports.strip_styles = function(string) {
  var replace_function = function(str, p1, p2, p3) {
    return sprintf('%s', p3);
  };
...
exports.puts = function() {
  ...
  for (var i = 0; i < arguments.length; i++) {
    if (supports_styles) {
      ...
      sys.puts(exports.strip_styles(arguments[i]));
    }
  }
};
