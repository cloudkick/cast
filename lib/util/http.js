 ...
 * @param {String} method What kind of request to perform (GET or POST)
 ...
 */
exports.get_response = function(url, method, parse_json, callback) {
  var host, port, secure, path;
  var data_buffer, data;

  if (!misc.in_array(method, ['GET', 'POST'])) {
    return callback(new Error(sprintf('Invalid method: %s', method)));
  }

  var url_object = urllib.parse(url);

  host = url_object.hostname;
  path = url_object.pathname || '/';

  secure = (url_object.protocol === 'https:') || false;
  port = url_object.port || (secure === true ? 443 : 80);
