...
function getRemoteCert(remote, callback) {
  ...
  function acceptBodyString(msg, maxBytes, callback) {
    var body = '';
...
function returnError(res, code, err, msg) {
  // TODO: eventually drop 'code' argument entirely
  if (!err) {
    err = code;
    if (err instanceof jobs.NotFoundError) {
      code = 404;
    } else if (err instanceof jobs.AlreadyExistsError) {
      code = 409;
    } else {
      code = 500;
    }
