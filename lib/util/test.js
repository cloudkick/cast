...
/* 
 * Return the Cast HTTP server.
 *
 * @param {String} services_http_root HTTP services root path (optional)
 * @param {Array} apps An array of the available services.
 *
 * @return {Object} HTTP server instances 
 */
var getServer = function(services_http_root, apps) {
  return require('services/http')._serverOnly(services_http_root, apps);
}
...
var run_test_tcp_server = function(ip_address, port, response_dictionary, wait_for_end,
  ...
  var send_response = function(data, stream) {
    ...
    for (regexp in regexp_match_objects) {
  ...
  var handle_connection = function(stream) {
    var self = this;
    ...
    stream.on('end', function() {
      var in_stream = self._streams.indexOf(this);
      ...
      if (in_stream !== -1) {
        self._streams.splice(in_stream, 1);
      }
  ...
  if (process.version.indexOf('v0.2')) {
      server_options = { 'allowHalfOpen': true };
  }
  else {
      server_options = null;
  }
  ...
  if (server_options) {
      tcp_server = net.createServer(server_options, handle_connection);
  }
  else {
      tcp_server = net.createServer(handle_connection);
  }

  tcp_server = tcp_server.listen(_port, _ip_address, function() {
    this._streams = [];
    callback.call(this);
  });
