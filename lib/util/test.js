 ...
 * @return {Object} params Optional params to be urlencoded ito the body.
 */
function getReqObject(url, method, params) {
  url = sprintf('/%s%s', httpConstants.CURRENT_API_VERSION, url);
  ...
  if (params) {
    req.body = querystring.stringify(params);
    req.headers = {
...
/**
 * Run a method which returns a job and set up handlers for success and error
 * event.
 *
 * @param {Function} method Method to call.
 * @param {Object} context Context in which the method is called.
 * @params {Array} args Arguments with which the function is called.
 * @param {Object} assert Assert object.
 * @param {Function} Callback called on end.
 */
function runJobMethod(method, context, args, assert, callback) {
  var j = method.apply(context, args);
...
exports.runJobMethod = runJobMethod;
