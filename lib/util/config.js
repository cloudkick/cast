...
exports.setupAgent = function(cb) {
  function validateAgentConfig(callback) {
    var conf = exports.currentConfig;
    var vmsg = 'SSL client verification is enabled, but CA certificate ';
    if (conf['ssl_enabled'] && conf['verify_client_cert']) {
      if (!conf['ssl_ca_cert']) {
        callback(new Error(vmsg + 'path not specified'));
        return;
      }
      fs.stat(conf['ssl_ca_cert'], function(err, stats) {
        if (err) {
          err = new Error(vmsg + 'not found at ' + conf['ssl_ca_cert']);
        } else if (!stats.isFile()) {
          err = new Error(vmsg + 'is not a file: ' + conf['ssl_ca_cert']);
        }
        callback(err);
        return;
      });
