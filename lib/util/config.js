...
var defaults = {
  ...
  'ca_dir': 'ca', // Stores CA certificate, key and generated pairs
  ...
  'ssl_ca_key': 'ca.key',       // Path to ssl CA key relative to ca_dir
  'ssl_ca_serial': 'ca.srl',    // Path to ssl CA serial number file relative
                                //  to ca_dir
  'ssl_ca_outdir': 'out',       // Path to ssl CA output dir relative to ca_dir
...
exports.setupAgent = function(callback) {
  function validateAgentConfig(callback) {
    ...
    if (!conf['ssl_enabled'] && !conf['secret']) {
      err = new Error('SSL is disabled, but no \'secret\' has been provided');
    } else if (!conf['verify_client_certificate'] && !conf['secret']) {
      err = new Error('Either \'verify_client_certificate\' must be enabled ' +
...
exports.setupClient = function(callback) {
  function validateClientConfig(callback) {
    callback();
  }

  setup(validateClientConfig, callback);
};
