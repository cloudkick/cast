...
var defaults = {
  ...
  'service_dir_enabled': null, // Directory in which the enabled runit services reside. This path can be relative
                               // or absolute. If it is relative, it is joined with the data_root path.
...
exports.setup = function(cb) {
  function join_paths(callback) {
    ...
    var service_dir_enabled, path1_setting, path2_setting, path1, path2;

    // If the service_dir_enabled option is defined and it's not absolute, add it to the paths_to_join
    // array
    service_dir_enabled = exports.config_current.service_dir_enabled;
    if (service_dir_enabled && service_dir_enabled[0] !== '/') {
      paths_to_join.push(['data_root', 'service_dir_enabled']);
    }
  ...
  function resolve_via_norris(callback, err) {
    ...
    norris.get(function(facts) {
      ...
      if (exports.config_current.service_dir_enabled === null) {
        exports.config_current.service_dir_enabled = facts.runit_services_path;
      }
