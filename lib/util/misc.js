 ...
 */
exports.get_bundle_name = function(full_bundle_name) {
  var splitted, bundle_name;

  splitted = full_bundle_name.split('@');

  if (splitted.length !== 2) {
    throw new exports.Errorf('Could not extract bundle name from the provided string: %s', full_bundle_name);
  }
...
/*
 * Return instance name and path.
 *
 * @param {String} full_bundle_name Bundle name which is returned by the get_full_bundle_name function.
 * @param {Number} instance_number Instance number.
 * @return {Array} First item is the instance name and the second item is instance path.
 */
exports.get_instance_name_and_path = function(full_bundle_name, instance_number) {
  var instance_name, instance_path, bundle_name;

  bundle_name = exports.get_bundle_name(full_bundle_name);

  instance_name = exports.get_valid_instance_name(full_bundle_name, instance_number);
  instance_path = path.join(config.get().app_dir, bundle_name, instance_name);
 ...
 */
exports.get_unix_timestamp = function(date) {
  var date_to_format = date || new Date();

  return Math.round(date_to_format / 1000);
};
