...
exports.is_valid_bundle_version = function(version) {
  var version_regexp;
  var valid_versions = [/^\d+.\d+.\d+$/, // major.minor.patch
                    /^(19|20)\d\d(0[1-9]|1[012])(0[1-9]|[12][0-9]|3[01])$/, // date (e.g. 20100810)
                    /^(19|20)\d\d(0[1-9]|1[012])(0[1-9]|[12][0-9]|3[01]).\b([a-f0-9]{40})\b$/ // date.githash (e.g. 20100912.d261151fad5b2ce95a2281a70fed2c6dab221731)
  ];

  for (var i = 0; i < valid_versions.length; i++) {
    version_regexp = valid_versions[i];
    if (version.match(version_regexp)) {
      return true;
    }
  }
