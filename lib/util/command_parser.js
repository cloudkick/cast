 ...
 */
CommandParser.prototype.add_global_option = function(option_name, option_object) {
  if (this._global_options.hasOwnProperty(option_name)) {
 ...
 *
 * @param {Object} options_map Mapping of global option names to objects
 */
CommandParser.prototype.add_global_options = function(option_map) {
  var option_name;
  for (option_name in option_map) {
    if (option_map.hasOwnProperty(option_name)) {
      this.add_global_option(option_name, option_map[option_name]);
    }
  }
};
...
CommandParser.prototype.merge_global_options = function(config) {
  var i, option_name;
  if (config.uses_global_options) {
    if (!config.options) {
      config.options = [];
    }
    for (i = 0; i < config.uses_global_options.length; i++) {
      option_name = config.uses_global_options[i];
      if (!this._global_options[option_name]) {
        throw new Error("Global option '" + option_name + "' is undefined");
      }
      config.options.push(this._global_options[option_name]);
    }
  }
};
...
CommandParser.prototype.parse = function(argv) {
  ...
  var options, strings_options, key, value, option, dest, option_name;
  var i, j;
  ...
  this.merge_global_options(config);
