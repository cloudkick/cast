...
CommandParser.prototype.parse = function(argv) {
  ...
  var options, strings_options, key, value;
  var i, j;
    ...
    if (strings_options.hasOwnProperty(key)) {
      switch(strings_options[key].action) {
        case 'store':
          ...
          arguments_dictionary[strings_options[key].dest] = value;
          break;
        ...
        case 'append':
          ...
          if (!arguments_dictionary.hasOwnProperty(key)) {
            arguments_dictionary[key] = [];
          }
          arguments_dictionary[strings_options[key].dest].push(value);
          break;
        ...
        case 'store_true':
          ...
          arguments_dictionary[strings_options[key].dest] = true;
          break;
        ...
        case 'store_false':
          ...
          arguments_dictionary[strings_options[key].dest] = false;
          break;
...
CommandParser.prototype.get_command_usage_string = function(command, sub_command) {
  ...
  arguments_string = this.get_command_arguments_string(command, sub_command, true);
  usage_string = sprintf('%s %s %s', this.binary, this.get_command_string(command, sub_command), arguments_string);

  return usage_string;
};

CommandParser.prototype.get_command_arguments_string = function(command, sub_command, options) {
  var i;
  var arguments_string = '';
  var terminator = ' ';
  var config = this.get_module_for_command(command, sub_command).config;

  if (options && config.switches.length > 0) {
