...
HTTPCheck.prototype.run = function(callback) {
  log.info(sprintf('Running HTTP check (URL: %s, type: %s, match_value: %s)',
           ...
           this.checkArguments.matchValue));

  var self = this;
  var result = new CheckResult();
  var timeout = false;

  var client = http.createClient(self.checkArguments.port,
                                 self.checkArguments.host,
                                 self.checkArguments.secure);

  var request = client.request('GET',
                               self.checkArguments.path,
  ...
  request.on('response', function(response) {
    if (self.checkArguments.type === config.types.STATUS_CODE_MATCH) {
      ...
      if (statusCode === self.checkArguments.matchValue) {
...
HTTPCheck.prototype.formatArguments = function(requiredArguments, optionalArguments,
  ...
  if (formattedArguments.type === config.types.STATUS_CODE_MATCH) {
    formattedArguments.matchValue = parseInt(formattedArguments.matchValue, 10);
  }
