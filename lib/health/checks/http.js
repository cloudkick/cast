...
HTTPCheck.prototype.run = function(callback) {
  log.info(sprintf('Running HTTP check (URL: %s, type: %s, match_value: %s)',
           this.check_arguments.url,
           this.check_arguments.type,
  ...
  var client = http.createClient(self.check_arguments.port,
                                 self.check_arguments.host,
                                 self.check_arguments.secure);

  var request = client.request('GET',
                               self.check_arguments.path,
                               {'host': self.check_arguments.host});
  ...
  client.on('end', function() {
    ...
    if (request.connection._writeQueue.length > 0) {
...
HTTPCheck.prototype.format_arguments = function(required_arguments, optional_arguments,
