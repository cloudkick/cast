...
var config = {
  ...
  'optional_arguments': ['match_value', 'command', 'connect_timeout', 'idle_timeout'],
  'default_values': {
...
function TCPCheck(check_arguments) {
  Check.call(this, config.name, config.required_arguments,
             config.optional_arguments, config.default_values, check_arguments);
}
...
TCPCheck.prototype.run = function(callback) {
  var connect_timeout_id;
  var got_response = false;

  log.info(sprintf('Running TCP check (IP: %s, port: %s)',
                   this.check_arguments.ip_address, this.check_arguments.port));

  var self = this;
  var result = new CheckResult();

  var connection = net.createConnection(self.check_arguments.port, self.check_arguments.ip_address);

  var connect_timeout = function() {
    // Node doesn't allow you to set connection timeout, so the underlying file description
    // is manually destroyed if the connection cannot be established after connect_timeout
    // milliseconds.
    if (!got_response) {
      got_response = true;
      ...
      result.details = sprintf('Check failed - connection timed out after %s seconds',
                               (self.check_arguments.connect_timeout / 1000));
  ...
  var clear_connect_timeout = function() {
    if (!got_response && connect_timeout_id) {
      got_response = true;
      clearTimeout(connect_timeout_id);
    }
  };

  connect_timeout_id = setTimeout(connect_timeout, self.check_arguments.connect_timeout);

  connection.on('connect', function() {
    clear_connect_timeout();
    connection.setTimeout(self.check_arguments.idle_timeout);
    ...
    if (self.check_arguments.type === config.types.CONNECTION_CHECK) {
      ...
      result.details = sprintf('Successfully established connection to IP %s port %s',
                               self.check_arguments.ip_address,
...
TCPCheck.prototype.format_arguments = function(required_arguments, optional_arguments,
                                               default_values, check_arguments)
{
  var formatted_arguments = Check.prototype.format_arguments.call(this, required_arguments,
                                                                  optional_arguments, default_values,
                                                                  check_arguments);

  if (formatted_arguments.type !== config.types.CONNECTION_CHECK &&
      formatted_arguments.type !== config.types.RESPONSE_REGEX_MATCH) {
