...
TCPCheck.prototype.run = function(callback) {
  ...
  log.info(sprintf('Running TCP check (IP: %s, port: %s)',
                   this.checkArguments.ipAddress, this.checkArguments.port));

  var self = this;
  var result = new CheckResult();

  var connection = net.createConnection(self.checkArguments.port, self.checkArguments.ipAddress);
  ...
  var connectTimeout = function() {
    ...
    if (!gotResponse) {
      ...
      result.details = sprintf('Check failed - connection timed out after %s seconds',
                               (self.checkArguments.connectTimeout / 1000));
  ...
  connectTimeoutId = setTimeout(connectTimeout, self.checkArguments.connectTimeout);

  connection.on('connect', function() {
    clearConnectTimeout();
    connection.setTimeout(self.checkArguments.idleTimeout);
    ...
    if (self.checkArguments.type === config.types.CONNECTION_CHECK) {
      ...
      connection.on('end', function() {
        ...
        if (body.match(self.checkArguments.matchValue)) {
          result.status = CheckStatus.SUCCESS;
          result.details = sprintf('The response body matched the regular expression: %s',
                                  self.checkArguments.matchValue.toString());
        }
        else {
          result.status = CheckStatus.ERROR;
                                               ...
                                               defaultValues, checkArguments)
{
  ...
  if (formattedArguments.type === config.types.RESPONSE_REGEX_MATCH) {
    formattedArguments.matchValue = new RegExp(formattedArguments.matchValue);
  }
