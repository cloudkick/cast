...
TCPCheck.prototype.run = function(callback) {
  ...
  var clearConnectTimeout = function() {
    if (!gotResponse && connect_timeoutId) {
      gotResponse = true;
      clearTimeout(connect_timeoutId);
    }
  };

  connect_timeoutId = setTimeout(connect_timeout, self.checkArguments.connect_timeout);
