...
/**
 * Tail a service's log file.
 * @param {Number} bytes The number of most recent bytes to read.
 * @param {Boolean} follow Whether to follow the log.
 * @param {Function} callback A callback fired with (err, data, unsubscribe).
 */
SupervisedService.prototype.tailLogFile = function(bytes, follow, callback) {
  tail.tailFile(this.getLogPath(), bytes, follow, callback);
};
...
function SupervisedServiceManager(pathAvailable, pathEnabled) {
  ...
  this._validActions = ['isEnabled', 'start', 'stop', 'restart',
...
SupervisedServiceManager.prototype.runAction = function(serviceName, action, callback) {
  if (!misc.inArray(action, this._validActions)) {
