 ...
 * Currently only runit is supported, but we also plan to support supervisior at
 * some point.
 */
var availableManagers = {
  ...
  // Mock manager, only used for tests
  'mock': {
    service: require('service_management/mock').MockService,
    manager: require('service_management/mock').MockServiceManager,
    getManager: require('service_management/mock').getManager,
    configDefaults: require('service_management/mock').configDefaults
  }
};
...
exports.getDefaultManager = function() {
  ...
  if (!defaultManager) {
    ...
    defaultManager = getManager(serviceManager);
  }
