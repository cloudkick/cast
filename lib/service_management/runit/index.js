...
RunitService.prototype.enable = function(callback) {
  var self = this;
  var target = path.resolve(self.pathAvailable);

  // Remove "down" file
  function removeDown(callback) {
    flowctrl.callIgnoringError(fs.unlink, null,
                               path.join(target, 'down'),
                               async.apply(fs.symlink, target,
                                            self.pathEnabled, callback));
  }

  // Wait for the "control" file to be created
  function waitForEnable(callback) {
    self._waitForEnable(500, 7000, function(err, enabled) {
      if (!err && !enabled) {
        err = new Error('Unable to enable service, is runit running?');
      }
      callback(err);
    });
  }
  ...
  self.isEnabled(function(enabled) {
    if (!enabled) {
      async.series([removeDown, waitForEnable], callback);
    } else {
      callback();
    }
  });
...
/**
 * Wait for a service to be enabled. This is a horrible polling based solution.
 * @param {Number} interval Interval in ms on which to poll.
 * @param {Number} timeout  Time in ms to wait.
 * @param {Function} callback A callback fired with (err, isEnabled).
 */
RunitService.prototype._waitForEnable = function(interval, timeout, callback) {
  var self = this;
  var timeSpent = 0;
  var cpath = path.join(this.pathAvailable, 'supervise', 'control');
