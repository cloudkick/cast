...
function RunitService(basedir, name) {
  SupervisedService.call(this, basedir, name);

  this.availpath = path.join(this.basedir, 'available', this.name);
  this.enablepath = path.join(this.basedir, 'enabled', this.name);
}
...
RunitService.prototype.get_log_path = function() {
  return path.join(this.availpath, 'log', 'main', 'current');
};
...
RunitService.prototype._control = function(cchar, cb) {
  ...
  this.is_enabled(function(enabled) {
    ...
    var cpath = path.join(self.availpath, 'supervise', '_control');
    ...
    fs.open(cpath, constants.O_WRONLY, 0700, function(err, fd) {
      if (err) {
        return cb(new Error('_control file not yet created'));
      }
 ...
 * @this
 */
function RunitServiceManager(dir) {
  SupervisedServiceManager.call(this, dir);

  this.availdir = path.join(this.services_path, 'available');
  this.enabldir = path.join(this.services_path, 'enabled');
}
...
RunitServiceManager.prototype.get_service = function(name, callback) {
  ...
  fs.stat(path.join(self.availdir, name), function(err, stats) {
    if (err) {
      ...
      return callback(null, new RunitService(self.services_path, name));
    }
  });
};
...
RunitServiceManager.prototype.get_service_template = function(service_name, service_path, entry_file, application_type,
                                                              callback) {
  var runit_templates = require('service_management/runit/templates/base');

  runit_templates.get_application_template(service_name, service_path, entry_file, application_type,
 ...
 */
var get_manager = function(base_dir) {
  var _base_dir = base_dir || path.join(config.get().service_dir);

  return new RunitServiceManager(_base_dir);
};
