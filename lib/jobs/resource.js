...
/**
 * Retrieve a list of resources of this type.
 * @param {Function} callback A callback fired with (err, list).
 */
 ...
 * @return {String} The serializer type for use by swiz.
 */
...
/**
 * Perform a "synchronized" check of whether a resource exists. This will
 * _waitToCheck then perform an existence check. If there was no existence
 * error then the caller gets back afterCheck from the callback and is
 * responsible for calling deref(), otherwise afterCheck and deref will be
 * called before the callback is fired. TODO: This needs a better name.
 * @param {Function} callback A callback fired with (err, afterCheck).
 */
Resource.prototype.alive = function(callback) {
  var self = this;
 ...
 */
Resource.prototype.getSerialized = function(callback) {
  var self = this;
  ...
  if (this._running) {
    ...
    self.alive(function(err, afterCheck) {
      if (err) {
        callback(err);
      } else {
...
Resource.prototype.enqueueJob = function(job, callback) {
  ...
  this._waitToCheck(function(afterCheck) {
    ...
    self.willExist(function(willExist) {
      ...
      job.on('_error', function() {
        self._deref(true);
      });

      job.on('_success', function() {
        self._deref(true);
      });
 ...
 * @return {Error} The appropriate error.
 */
Resource.prototype.getExistenceError = function(exists) {
  if (exists) {
    return new errors.AlreadyExistsError(this.constructor.name, this.name);
  } else {
    return new errors.NotFoundError(this.constructor.name, this.name);
  }
};


Resource.prototype.deref = function() {
  this._deref();
};
 ...
 */
Resource.prototype._deref = function(queued) {
  this._refCount--;
 ...
 * @return {String} The path to the directory containing resource sof this
 *     type.
 */
 ...
 * @return {String} Path to the root of this resource.
 */
...
ResourceManager.prototype.get = function(name, callback) {
  ...
  resource.getSerialized(callback);
};


/**
 * Retrieve a "live" resource object. The resource will not be locked, and
 * jobs may be executing on it, so basically be very careful what you do with
 * it. You also need to call deref() on the resource you when you are done.
 * @param {String} name The name of the resource to retrieve.
 * @param {Function} callback A callback fired with (err, resource).
 */
ResourceManager.prototype.getLive = function(name, callback) {
  var self = this;
  var resource = this._getOrCreate(name);
  resource.alive(function(err, afterCheck) {
    if (err) {
      callback(err);
    } else {
