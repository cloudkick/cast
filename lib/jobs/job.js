...
function Job(args) {
  ...
  this.on('_success', function(result) {
    self._resource = undefined;
    self.timeEnded = new Date().toUTCString();
  ...
  // Prevent node from throwing errors that it thinks are uncaught
  // Note: errors aren't actually uncaught, as they originated through the
  // '_error' event which we do listen for.
  ...
  this.on('start', function() {
    self.timeStarted = new Date().toUTCString();
    self.status = 'running';
  });
}
 ...
 * @return {Array} Serializer definitions.
 */
Job.prototype.getSerializerDefs = function() {
  return  {
    'Job': [
      ['id', {src: 'id', type: 'string'}],
      ['jobType', {src: 'getJobTypeName', type: 'string'}],
      ['jobArgs', {src: 'args', type: 'array<string>'}],
      ['queued_at', {src: 'timeQueued', type: 'date'}],
      ['started_at', {src: 'timeStarted', type: 'date'}],
      ['ended_at', {src: 'timeStarted', type: 'date'}],
...
/**
 * Retrieve the name of the class of job this is.
 * @param {Function} callback A callback fired with (err, name).
 */
Job.prototype.getJobTypeName = function(callback) {
  callback(null, this.constructor.name);
};
 ...
 * @return {Boolean} Whether the resource for this job must exist.
 */
 ...
 * @return {Boolean} Whether the resource for this job must not exist.
 */
 ...
 * @return {Boolean} Whether this job will create its resource.
 */
 ...
 * @return {Boolean} Whether this job will delete its resource.
 */
...
Job.prototype.enqueueFor = function(resource) {
  ...
  this._resource.enqueueJob(this, function(err) {
    ...
    self.timeQueued = new Date().toUTCString();
    self.status = 'queued';
    self.emit('ready');
  });
};
 ...
 * @return {Boolean} whether this job can operate on the resource.
 */
...
Job.prototype.start = function() {
  ...
  this._resource.exists(function(exists) {
    if (!self.canHandleExistence(exists)) {
      self.emit('_error', self._resource.getExistenceError(exists));
      return;
    }
