...
var get_application_template = function(instance_name, instance_path, entry_file, application_type, callback)
{
  ...
  try {
    get_runit_template = require(sprintf('runit/templates/%s', application_type)).get_template;
  }
  catch (error) {
    return callback(new Error(sprintf('Cannot load template module for application type %s', application_type)));
  }

  get_runit_template(instance_name, instance_path, entry_file, function(error, template) {
    if (error) {
