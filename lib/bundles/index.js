...
BundleManager.prototype.cleanApplication = function(name, callback) {
  ...
  function cleanAppBundleDir(callback) {
    fs.unlink(self.getAppBundlePath(name), function(err) {
  ...
  function cleanAppExtractedDir(callback) {
    fs.unlink(self.getAppExtractedPath(name), function(err) {
      if (err && err.code === 'ENOTEMPTY') {
        err = undefined;
      }
      callback(err);
...
BundleManager.prototype.remove = function(name, version, callback) {
  ...
  async.series([
    ...
    self.cleanApplication.bind(self, name),

    // Unlock BundleManager
    self.releaseLock.bind(self),
