...
function PluginManager() {
  /* Store pluginName to plugin manifest mappings */
  this._availablePlugins = {};

  /* Store plugin name to plugin settings mappings */
  ...
  async.waterfall([
    ...
    function readPluginsManifest(files, callback) {
      async.forEach(files, function(file, callback) {
        ...
        if (self._availablePlugins.hasOwnProperty(file)) {
          // Manifest for this file is already cached
          callback();
          return;
        }

        fs.stats(pluginPath, function(err, stats) {
          if (err) {
            log.error('Error while stating plugin file / directory: %s',
                      err.message);
            callback();
            return;
         }
         ...
         self._readPluginManifest(pluginPath, function(err, manifestObj) {
           if (err) {
             log.error('Error while reading plugin manifest: %s', err.message);
             callback();
             return;
           }
