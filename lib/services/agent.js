...
Agent.prototype.connect_to_endpoint = function() {
  ...
  this.connection.on('connect', function() {
    ...
    log.info(sprintf('Connected to the endpoint %s:%s', endpoint_host,
                                                        endpoint_port));
    // Clear the connect timeout, reset the connection delay timer and clear the restarting flag
    self.clear_connect_timeout();
...
/*
 * Clear the connection timeout handler and set the connected property to true.
 *
 * This function is called after the connection is successfully established or if the connection fails
 * because of a different reason then timeout (connection refused, etc.)
 */
 ...
 */
Agent.prototype.handle_cmd_run_check = function(check_name, token, payload_len, payload) {
  // @TODO: Run a check in a separate check process
};
