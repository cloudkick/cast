...
var WorkerPool = require('extern/node-worker/index').worker_pool.WorkerPool;
...
function Agent() {
  ...
  this._endpoint_port = null;
  this._endpoint_host = null;

  this._connect_delay = constants.INITIAL_CONNECT_DELAY;

  this._ping_sent_count = 0;
  this._pong_got_count = 0;
  this._last_ping_time = 0;
  this._buffer = '';
  ...
  this._pending_commands_queue = [];

  this._connected = false;
  this._connect_timeout_id = null;
...
Agent.prototype.start = function(port, host) {
  ...
  this._endpoint_port = port || config.get().agent_endpoint_port;
  this._endpoint_host = host || config.get().agent_endpoint_host;
...
Agent.prototype.connect_to_endpoint = function() {
  ...
  this._connection = net.createConnection(this._endpoint_port, this._endpoint_host);

  var handle_error = function(err) {
    log.info(sprintf('Connection to %s:%s failed, reason: %s', self._endpoint_host,
                                                               self._endpoint_port,
                                                               err.message));
  ...
  this._connect_timeout_id = setTimeout(function() {
  ...
  this._connection.on('connect', function() {
    ...
    log.info(sprintf('Connected to the endpoint %s:%s', self._endpoint_host,
                                                        self._endpoint_port));
    ...
    self._ping_interval_id = setInterval(function() {
      self.send_cmd_ping();
    ...
    self._connection.on('data', handle_data);
    self._connection.on('end', handle_end);
    self._connection.on('close', handle_close);
  });
};
...
Agent.prototype.reconnect_to_endpoint = function(now) {
  ...
  if (now === true) {
    ...
    connect_delay = this._connect_delay;
  }
...
Agent.prototype.increase_backoff_delay = function() {
  this._connect_delay += Math.ceil(Math.random() * 2000);
};
...
Agent.prototype.handle_connect_timeout = function() {
  if (!this._connected) {
    log.info(sprintf('Connection to %s:%s failed, reason: timeout', this._endpoint_host,
                                                                    this._endpoint_port));
...
Agent.prototype.clear_connect_timeout = function() {
  if (!this._connected && this._connect_timeout_id) {
    clearTimeout(this._connect_timeout_id);
  }
};
...
Agent.prototype.reset_ping_counters_and_clear_interval = function() {
  ...
  clearInterval(this._ping_interval_id);
};
...
Agent.prototype.send_command = function(command, command_arguments) {
  ...
  this._connection.write(command_string + '\n');
...
Agent.prototype.queue_outgoing_command = function(command, command_arguments) {
  ...
  if (this._pending_commands_queue.length >= constants.COMMAND_QUEUE_SIZE) {
    this._pending_commands_queue.pop();
  }

  this._pending_commands_queue.push([ command, command_arguments ]);
};
...
Agent.prototype.replay_queued_commands = function() {
  var i, command;
  var pending_commands_length = this._pending_commands_queue.length;
  ...
  for (i = 0; i < pending_commands_length; i++) {
    command = this._pending_commands_queue.shift();
...
Agent.prototype.handle_command = function(command, command_arguments) {
  ...
  this.log_command(command, 'incoming', command_arguments);
...
Agent.prototype.send_cmd_ping = function() {
  ...
  var time_diff = current_time - this._last_ping_time;

  if ((this._ping_sent_count - this._pong_got_count) >= constants.MAX_MISSED_PONGS) {
  ...
  this._last_ping_time = current_time;
  this._ping_sent_count++;
};
