...
function extract_path() {
  return path.join(config.get().data_root, 'extracted');
}

/**
 * Attempt to get a lock on a deployment. Callback receives an error if the
 * deployment is already locked, otherwise it gets an fd for use in releasing
 * the lock.
 * @param {String} deployment The name of the deployment to get the lock on.
 * @param {Function} cb The callback, gets two arguments, (error, fd).
 */
function get_deployment_lock(deployment, cb) {
  var lockpath = path.join(extract_path(), deployment, '.deplock');
  fs.open(lockpath, process.O_EXCL | process.O_CREAT, 644, cb);
}

/**
 * Release a deployment lock.
 * @param {String} deployment The name of the deployment the lock is on
 * @param {Number} fd The file descriptor from when the lock was achieved.
 * @param {Function} cb The callback, takes a possible error.
 */
function release_deployment_lock(deployment, fd, cb) {
  var lockpath = path.join(extract_path(), deployment, '.deplock');
  if (fd) {
...
function do_deploy(req, res, deployment, args) {
  ...
  async.series([
    ...
    function(callback) {
      get_deployment_lock(deployment, function(err, fd) {
        if (err) {
          err = new Error("Deployment already in progress");
          err.code = 503;
        }
        else {
          deplock = fd;
  ...
  function(err) {
    release_deployment_lock(deployment, deplock, function(lockerr) {
      if (lockerr) {
        log.err("Unable to release lock on " + deployment + ": " + err.message);
      }
      if (err) {
        log.err("Deployment error: " + err.message);
