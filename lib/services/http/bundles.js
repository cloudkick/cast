...
/**
 * Given a bundle name and a bundle string, return a validated and normalized
 * path to a (not necessarily existing) file. Return false on invalid paths.
 *
...
function upload(req, res, bundle, file) {
  ...
  if (!p) {
    return http.return_erro(res, "404", "File not found");
  }
...
function list_files(req, res, bundle) {
  var p = toPath(bundle);
  directory_or_404(p, res, function() {
    fs.readdir(p, function(err, files) {
      if (err) {
        // This could happen in the course of normal operations, so 404
        log.err("Error reading directory: " + err.message);
        return http.return_error(res, 404, "File not found");
      }
