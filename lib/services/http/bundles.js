...
var extract_tarball = require('util/extract').extract_tarball;
...
function upload(req, res, bundle, file) {
  ...
  async.series([
    ...
    function(callback) {
      ...
      pumpfilein_via_tempfile(p, req, received, completed, callback);
    }
  ],
...
function extract_bundle_file(req, res, bundle, file) {
  ...
  file_or_404(fpath, res, function(stats) {
    ...
    log.debug("Extracting " + fpath + " to " + extpath);
    async.series([
