 ...
 */
var filePath = function(bundle, file) {
 ...
 */
var bundlePath = function(bundle) {
  var bundleroot = path.join(config.get()['bundle_dir']);
  var bpath = path.normalize(path.join(bundleroot, bundle));
 ...
 */
var extractPath = function(bundle, file) {
 ...
 */
var tempfileInDirectory = function(filename) {
  var randname;
 ...
 *                      its only argument.
 */
var pathOr404 = function(p, res, callback) {
  if (!p) {
 ...
 *                      its only argument.
 */
var fileOr404 = function(p, res, callback) {
 ...
 */
var upload = function(req, res, bundle, file) {
 ...
 */
var download = function(req, res, bundle, file) {
 ...
 */
var remove = function(req, res, bundle, file) {
  var fpath = filePath(bundle, file);
  fileOr404(fpath, res, function() {
    fs.unlink(fpath, function(err) {
      if (err) {
        http.returnError(res, 500, 'Error removing bundle file: ' + err.message);
        return;
      }
      res.writeHead(204, {});
      res.end();
 ...
 * @param {String} bundle The name of the bundle to delete from.
 */
var listFiles = function(req, res, bundle) {
 ...
 * @param {http.ServerResponse} res The HTTP response to respond to.
 */
var listBundles = function(req, res) {
  var pathExtracted = config.get()['extracted_dir'];

  var getBundleVersions = function(bundleName, callback) {
