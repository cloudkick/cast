 ...
 *
 * @return The normalized file path, or false on invalid paths.
 */
function file_path(bundle, file) {
  if (file.indexOf(bundle) !== 0 || !file.match(/\.tar\.gz$/)) {
    return false;
  }
  var bundleroot = path.join(config.get().data_root, config.get().bundle_dir);
 ...
 * @param {String} bundle The name of the bundle.
 *
 * @return The normalized bundle path, or false on invalid paths.
 */
...
function extract_path(bundle, file) {
  var exroot = path.join(config.get().data_root, config.get().extracted_dir);
 ...
 * @return A path to a temporary file in the same directory as filename.
 */
