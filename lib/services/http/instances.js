...
function create_instance(req, res, bundle_name) {
  var instance_name, instance_path, manifest_object, ignored_files;
  ...
  async.series([
    ...
    function(callback) {
      ignored_files = manifest_object.template_files.concat(manifest_object.data_files);

      fsutil.hard_link_files(extracted_bundle_path, instance_path, ignored_files, function(error) {
        if (error) {
          return callback(error);
        }
