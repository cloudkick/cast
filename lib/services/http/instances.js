...
function create_instance(req, res, bundle_name_full) {
  ...
  var extracted_bundle_path = path.join(config.get().data_root, config.get().extracted_dir, bundle_name,
                                        bundle_name_full);
  ...
  async.series([
    ...
    function(callback) {
      deployment.get_available_instances(bundle_name, bundle_version, function(error, instances) {
        ...
        application_data_path = path.join(config.get().data_root, config.get().data_dir, bundle_name);
