...
function create_instance(req, res, bundle_name_full) {
  var application_instance_path, instance_name, instance_path, manifest_object, ignored_paths;
  ...
  var extracted_bundle_path = path.join(config.get().data_root, config.get().extracted_dir, bundle_name, bundle_name_full);
  ...
  async.series([
    ...
    function(callback) {
      deployment.get_available_instances(bundle_name, bundle_version, function(error, instances) {
        ...
        application_instance_path = path.join(config.get().data_root, config.get().app_dir,
                                              bundle_name);
    ...
    function(callback) {
      ...
      deployment.realize_application_templates(manifest_object, instance_data, extracted_bundle_path, instance_path, function(error) {
        if (error) {
          return callback(error);
        }
...
function enable_instance(req, res, bundle_name_full, instance_number) {
  var instance_name, instance_path, bundle_name, splitted;

  splitted = bundle_name_full.split('@');
  bundle_name = splitted[0];

  instance_name = misc.get_valid_instance_name(bundle_name_full, instance_number);
  instance_path = path.join(config.get().data_root, config.get().app_dir, bundle_name, instance_name);
