...
function tail_service(req, res, service) {
  var log_file;
  ...
  dir.get_service(service, function(err, svc) {
    ...
    log_file = path.join(dir.availdir, service, 'log', 'main', 'current');
    tail_file(log_file, 250, function (error, data, unsubscribe) {
      if (error) {
        return http.return_error(res, 500, error.message);
      }

      if (!head_written) {
        res.writeHead(200, { 'Content-Type': 'text/plain', 'stream': 'keep-alive' });
        head_written = true;
      }
