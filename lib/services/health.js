...
load = function() {
  function start_health() {
    ...
    log.info('health service started');
        ...
        health.add_check(new ScheduledCheck(new http_check.HTTPCheck({'url': 'http://www.google.com', 
                                        'type': http_check.config.types.STATUS_CODE_MATCH,
                                        'match_value': 200}), 2000));
    health.add_check(new ScheduledCheck(new http_check.HTTPCheck({'url': 'http://www.cloudkick.com', 
                                        'type': http_check.config.types.STATUS_CODE_MATCH,
                                        'match_value': 200}), 10000));
     health.add_check(new ScheduledCheck(new http_check.HTTPCheck({'url': 'http://www.cloudkick.com', 
                                        'type': http_check.config.types.STATUS_CODE_MATCH,
                                        'match_value': 200}), 1000));
                                        
    setTimeout(function () {
      //health.restart(4);
      health.pause_check(health.get_checks_array()[0].id);
    }, 4000);
  }
