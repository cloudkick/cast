 ...
 */
function Hook(type, name, hookPath, cwd, baseEnv) {
  this.type = type;
  this.path = hookPath;
  this.cwd = cwd;
...
Hook.prototype.execute = function(timeout, args, callback) {
  ...
  async.waterfall([
    ...
    function(callback) {
      if (!hookExists) {
        var err = new Errorf('hook %s does not exist', self.path);
 ...
 * {String} name Hook working directory.
 * {Object} baseEnv Base environment which is set for the hook.
 */
function InstanceHook(type, name, instanceVersionPath, baseEnv) {
  var hookPath = path.join(instanceVersionPath, '.cast-project', 'hooks',
                           name);
  Hook.call(this, type, name, hookPath, instanceVersionPath, baseEnv);
