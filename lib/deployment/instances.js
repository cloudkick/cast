...
Instance.prototype.destroy = function(callback) {
  ...
  function destroyService(bundleVersion, callback) {
    ...
    manager.getService(serviceName, function(err, service) {
      ...
      flowctrl.callIgnoringError(service.destroy, service, callback);
    });
  }

  var ops = [getBundleVersion, removeInstanceDirectory, destroyService];
...
Instance.prototype.upgrade = function(bundleVersion, callback) {
  ...
  var newInstanceVersionPath, oldServiceName, manifestObj;
  ...
  function getBundleVersion(callback) {
    self.getBundleVersion(function(err, version) {
      ...
      oldServiceName = misc.getFullBundleName(bundleName, version);
      callback();
    });
  }
  ...
  function disableOldService(callback) {
    var manager = serviceManagement.getDefaultManager().getManager();
    manager.getService(oldServiceName, function(err, service) {
      if (err) {
        callback(err);
        return;
      }
      ...
      service.disable(function(err) {
        // We ignore error at this stage, it is possible that the service was
        // already disabled.
        callback();
      });
    });
  ...
  function startNewService(callback) {
    ...
    // TODO: Do a rollback if the service is still reported as down after x
    // milliseconds.
    // TODO: Delete an old service on success.
  }

  var ops = [getBundleName, getBundleVersion, prepareVersion, retrieveManifestObject,
             createNewService, activateNewVersion, disableOldService, startNewService];
