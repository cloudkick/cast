...
Instance.prototype.exists = function(callback) {
  path.exists(this.root, callback);
};
 ...
 */
Instance.prototype.get_bundlename = function(callback) {
  var self = this;
  if (self._bundlename) {
    ...
    fs.readlink(bundle_link_path, function(err, target) {
      ...
      return callback(err, self._bundlename);
    });
  }
};
...
Instance.prototype.get_version_path = function(version, callback) {
  var self = this;
  self.get_bundlename(function(err, bundle_name) {
 ...
 *
 * @param {String} version    The version to get the path to
 * @param {Function} callback A callback that takes (err, version_bundle_path)
 */
Instance.prototype.get_version_bundle_path = function(version, callback) {
  var self = this;
  self.get_bundlename(function(err, bundle_name) {
    var bundle_name_full = bundle_name + '@' + version;
    var version_bundle_path = path.join(config.get().extracted_dir, bundle_name, bundle_name_full);
    return callback(err, version_bundle_path);
  });
};
...
Instance.prototype.prepare_version = function(version, callback) {
  ...
  async.series([
    ...
    function(callback) {
      self.get_bundlename(function(err, name) {
        bundle_name = name;
        bundle_name_full = bundle_name + '@' + version;
        extracted_bundle_path = path.join(config.get().extracted_dir, bundle_name, bundle_name_full);
        instance_version_path = path.join(self.root, 'versions', bundle_name_full);
        return callback(err);
      });
    },
...
Instance.prototype.activate_version = function(version, callback) {
  ...
  var new_version_link = path.join(self.path, 'new');
  var current_version_link = path.join(self.path, 'current');
  ...
  async.series([
    ...
    // Create the new link
    async.apply(fs.symlink, new_version_path, new_version_link),
