...
{
  var svc_path = runit.get_service_dir();
  var svc_avail_path = path.join(svc_path.availdir, instance_name);
  var svc_enabled_path = path.join(svc_path.enabldir, instance_name);

  async.parallel([
    async.apply(flowctrl.call_ignoring_error, fsutil.rmtree, instance_path),
    async.apply(flowctrl.call_ignoring_error, fsutil.rmtree, instance_data_path),
    async.apply(flowctrl.call_ignoring_error, fsutil.rmtree, svc_avail_path),
    async.apply(flowctrl.call_ignoring_error, fs.unlink, svc_enabled_path)
  ],
