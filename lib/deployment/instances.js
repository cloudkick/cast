 ...
 * @inheritdoc
 */
Instance.prototype.getSerializerDef = function() {
  return [
    ['name', {
      src: 'name',
      type: 'string'
 ...
 */
Instance.prototype._serviceAction = function(action, callback) {
  var manager = serviceManagement.getDefaultManager().getManager();
  this.getServiceName(function(err, serviceName) {
    if (err) {
      callback(err);
    } else {
      manager.serviceAction(serviceName, action, callback);
    }
  });
};
...
/**
 * Enable this instance's service.
 */
Instance.prototype.enableService = function(callback) {
  this._serviceAction('enable', callback);
};


/**
 * Disable this instance's service.
 */
Instance.prototype.disableService = function(callback) {
  this._serviceAction('disable', callback);
};


/**
 * Start this instance's service.
 */
Instance.prototype.startService = function(callback) {
  this._serviceAction('start', callback);
};


/**
 * Stop this instance's service.
 */
Instance.prototype.stopService = function(callback) {
  this._serviceAction('stop', callback);
};


/**
 * Restart this instance's service.
 * @param {Function} callback A callback fired with (err).
 */
Instance.prototype.restartService = function(callback) {
  this._serviceAction('restart', callback);
};
...
Instance.prototype.destroy = function(callback) {
  ...
  function removeInstanceDirectory(bundleVersion, callback) {
    flowctrl.callIgnoringError(fsutil.rmtree, null, self.getRoot(), function(err) {
      callback(null, bundleVersion);
    });
  }
