...
function createInstance(instanceName, bundleName, bundleVersion, enableService,
  ...
  async.series([
    ...
    function(callback) {
      deployServices.createService(instanceName, instanceVersionPath, manifestObj, callback);
    },
    ...
    function(callback) {
      ...
      manager.getService(instanceName, function(err, service) {
        if (err) {
          callback(err);
          return;
        }
        ...
        service.enable(function() {
          // Callback is called immediately but we delay starting of the service
          // for RUNIT_DELAY number of milliseconds so runit has enough time to
          // pick up new changes.
          callback();
