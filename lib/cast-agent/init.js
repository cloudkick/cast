  ...
  isFirstRun(function(err, firstRun) {
    ...
    if (firstRun) {
      ops.push(function(callback) {
        ...
        fsutil.ensureDirectory(config.get()['data_root'], function(err) {
          if (err) {
            callback(err);
            return;
          }

          pathsToCreate = [config.get()['data_dir'],
                             config.get()['service_dir_available'],
                             config.get()['bundle_dir'],
                             config.get()['extracted_dir'],
                             config.get()['app_dir']];

          async.forEachSeries(pathsToCreate,
            function(path_, callback) {
              fsutil.ensureDirectory(path_, callback);
            },
