...
var requirements = {
  ...
  'runit_path': [null, req.is_defined, 'runsvdir must be running before the agent can be started']
};
  ...
  ps.once(ps.AGENT_CONFIG_DONE, function() {
    ...
    req.meets_requirements(requirements, function(err, meet_requirements) {
      ...
      init.initialize(function(err) {
        var manager, service, service_module;
        ...
        // Add activated service manager to the services object
        manager = config.get().service_manager;
        services[manager] = 'service_management/' + manager + '/load';
