...
var requirements = {
  'node_version': ['0.4.0', req.compare_versions, 'Minimum node version must be 0.4.0'],
  'runsvdir_running': [
    null,
    req.is_defined,
  ...
  ps.once(ps.AGENT_CONFIG_DONE, function() {
    ...
    req.meets_requirements(requirements, function(err, meet_requirements) {
      ...
      init.initialize(function(err) {
        ...
        exports.date_started = new Date();
        ps.emit(ps.AGENT_STATE_START);
      });
    });
  });

  config.setup_agent(function(err) {
    if (err) {
