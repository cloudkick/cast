...
function makePathRelative(file, root, requireFile, callback) {
  ...
  function relativise() {
    if (file.indexOf(root) !== 0) {
      return callback(new Error(file + ' is not within ' + root));
    }
...
function handleCommand(args, parser, callback) {
  ...
  async.waterfall([
    ...
    // If an entryfile argument is not provided, ask user for it
    function(callback) {
      if (args.entryfile) {
        callback();
      }
      else {
        terminal.prompt('Path to the entry file:', false, false, null, function(entryFile) {
    ...
    function(callback) {
      ...
      terminal.prompt('Application Name:', false, false, null, function(name) {
        manifest.name = name;
        callback();
        return;
      });
    ...
    function(callback) {
      terminal.prompt('Application Description:', false, false, null, function(desc) {
    ...
    function(callback) {
      terminal.prompt('Application version:', false, '1.0.0', null, function(version) {
    ...
    function(callback) {
      ...
      terminal.prompt('Application Type:', apptypes, apptypes[0], null, function(type) {
        manifest.type = type;
        callback();
        return;
      });
    ...
    function(callback) {
      if (manifestConstants.HEALTH_CHECKS[manifest.type].length !== 0) {
        ...
        terminal.prompt(query, ['y', 'n'], 'y', null, function(resp) {
          if (resp === 'y') {
  ...
  function(err) {
    if (err) {
      ...
      callback(null, 'Manifest created');
    }
  });
}
