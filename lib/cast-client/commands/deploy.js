  ...
  function applicationBundleExists(name, version, callback) {
    ...
    async.waterfall([
      ...
      function filterResponse(res, callback) {
        ...
        function findMatchingBundle(appBundles, version) {
          var i, len, bundle, bundleVersion;

          for (i = 0, len = appBundles.length; i < len; i++) {
            bundle = appBundles[i];
            bundleVersion = bundles.getBundleVersion(bundleName, bundle);
        ...
        for (i = 0, len = apps.length; i < len; i++) {
          ...
          exists = findMatchingBundle(appBundles, version);
