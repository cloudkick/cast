  ...
  options: [
    {
      ...
      title: 'remote',
      action: 'store',
      desc: 'The remote to which the bundle should be uploaded'
    }
  ]
};
...
exports.handle_command = function(args)
{
  ...
  var bundledir = path.join(dotfiles.dot_cast_project_path(process.cwd()), 'tmp');
  ...
  async.series([
    ...
    function(callback) {
      ...
      // If a version was specified, make sure it exists
        ...
        var full_bundlename = misc.get_full_bundle_name(bundlename, version);
        fpath = path.join(bundledir, full_bundlename + '.tar.gz');
