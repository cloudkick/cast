 ...
 * @param {String} Application path to operate on.
 */
exports.handleCommand = function(args, parser, callback) {
  var applicationPath;
  ...
  path.exists(manifestFile, function(exists) {
    ...
    manifest.validateManifest(manifestFile, function(err, manifestObject) {
      var successMessage = 'Manifest file is valid.';

      if (err) {
        err = new Error(sprintf('Manifest file validation failed: %s', err.message));
      }
