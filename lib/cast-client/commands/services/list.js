  ...
  'options': [
    {
      ...
      title: 'remote',
      action: 'store',
      desc: 'The remote on which available services should be listed'
    }
  ]
};
...
function handle_command(args) {
  http.get_response(args.remote, '/services/', 'GET', true, function(error, response) {
    if (error) {
      error = error;
      return sys.puts(sprintf('Error: %s\n%s', error.message, error.stack));
    }
