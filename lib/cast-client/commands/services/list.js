...
var handleCommand = function(args) {
  http.getApiResponse(args.remote, '1.0', '/services/', 'GET', true, function(err, response) {
    if (err) {
    ...
    if (response.statusCode !== 200) {
      sys.puts(sprintf('HTTP Error, status code: %d, returned body: "%s"', response.statusCode,
                                                                           response.body));
      return;
    }
