...
function handle_command(args) {
  http.get_response(undefined, '/services/', 'GET', true, function(error, response) {
    if (error) {
      error = error;
      return sys.puts(sprintf('Error: %s', error.message));
    }
    if (response.code !== 200) {
      return sys.puts(sprintf('HTTP Error, %d returned, body: %s', response.code, response.message));
    }
