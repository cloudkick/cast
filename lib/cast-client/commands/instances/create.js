...
function handle_command(args) {
  ...
  var bundle_name_full = misc.get_full_bundle_name(args.bundle, args.version);
  var remote_path = sprintf('/instances/%s/%s/', bundle_name_full, instance_name);

  http.get_response(args.remote, remote_path, 'POST', true, function(error, response) {
    if (error) {
