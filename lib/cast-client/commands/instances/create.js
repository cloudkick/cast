  ...
  'options': [
    {
      ...
      dest: 'enable',
      action: 'store_true',
      desc: 'Enable and start the instance after it has been created'
    },
    {
      names: ['--remote', '-r'],
      dest: 'remote',
      title: 'remote',
      action: 'store',
      desc: 'The remote on which the instance should be created'
    }
  ]
};
...
function handle_command(args) {
  ...
  http.get_response(args.remote, sprintf('/instances/%s/', bundle_name_full), 'POST', true,
