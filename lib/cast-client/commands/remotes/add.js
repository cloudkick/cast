...
exports.handleCommand = function(args) {
  ...
  if (url.parse(args.url).protocol === 'https:') {
    calls = calls.concat([
      function(remotes, callback) {
        ...
        http.getServerSslCertInfo(args.url, function(err, certInfo) {
          if (err) {
            callback(err);
            return;
          }
      ...
      function(remotes, certInfo, callback) {
        ...
        sys.puts(sprintf('  Valid to: %s', certInfo.validTo));
  ...
  calls.push(function(remotes, fingerprint, callback) {
    ...
    if (isDefault) {
      for (name in remotes) {
        if (remotes.hasOwnProperty(name) && remotes[name].isDefault) {
          remotes[name].isDefault = false;
        }
      }
    }
