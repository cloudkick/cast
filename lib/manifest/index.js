 ...
 *                            with manifest_object as the second argument otherwise.
 */
exports.validate_manifest = function(manifest_path, callback) {
  exports.get_manifest_object(manifest_path, true, function(error, manifest_object) {
    if (error) {
      callback(error, null);
      return;
    }
    ...
    for (field in manifest_object) {
      if (manifest_object.hasOwnProperty(field)) {
    ...
    if (!misc.array_is_subset_of(manifest_fields, all_fields)) {
      ...
      callback(new Error(sprintf('Manifest contains invalid fields: %s', invalid_fields)));
      return;
    }
 ...
 *                            with manifest_object as the second argument otherwise.
 */
exports.get_manifest_object = function(manifest_path, merge_with_defaults, callback) {
  path.exists(manifest_path, function(exists) {
    if (!exists) {
      callback(new Error('Invalid path to the manifest file: ' + manifest_path), null);
      return;
    }
