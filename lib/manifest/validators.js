...
var is_valid_check = function(value, options, callback) {
  ...
  var index = misc.array_find(check_name, health.available_checks, null, function(item, needle) { return item[0] === needle; });
  if (index === false) {
  ...
  try {
    new check_module[check_class](value.arguments);
  }
  catch (error) {
    return callback(error);
  }
